...
sst1 <- subset(sst1,Type %in% c("Normal","Tumor"))


dcast(sst1,Case.number ~ Type,mean) %>%
  na.omit -> sst1.pairs
...
# Analysis of nuclei size in LS174T and OV90 clones ----

# Load nuclei size data

nucSizeLS <- read.delim("data/NucleiSizeLS.tsv",sep="\t",stringsAsFactors=F)

nucSizeLS$sample %>% factor(.,c("LS174T",paste("CLONE",1:14,sep = "_"))) -> nucSizeLS$sample

m_cv <- function(x) c(mean=mean(x),sd=sd(x),cv=sd(x)/mean(x))
aggregate(area ~ sample,nucSize,m_cv)


ggplot(nucSizeLS,aes(log(area))) + geom_density(aes(fill=sample),alpha=.2) 

if(CREATE_PLOTS) png("graphs/nucleiSize.png",6,4,units = "in",res=300)
ggplot(nucSizeLS) + geom_boxplot(aes(x=sample,y=area,fill=mSize[sample])) + 
...
nucSizeOV <- read.delim("data/NucleiSizeOV.tsv",sep="\t")
nucSizeOV$sample %>% factor(.,c("OV90",paste("CLONE",1:17,sep = "_"))) -> nucSizeOV$sample

mSizeOV <- tapply(nucSizeOV$area,nucSizeOV$sample,mean)

if(CREATE_PLOTS) png("graphs/nucleiSizeOV.png",6,4,units = "in",res=300)
ggplot(nucSizeOV) + geom_boxplot(aes(x=sample,y=area,fill=mSizeOV[sample])) + 
  scale_fill_gradient(name="Mean area",low="#FFAA55",high="#CC5522") + 
  xlab(NULL) + ylab("Nuclei area (μm2)") +
  theme(axis.text.x = element_text(vjust = .5,hjust=1,angle=90)) +
  ggtitle("Nuclei area in OV90 and derived clones")
if(CREATE_PLOTS) dev.off()

tukLS <- TukeyHSD(aov(area ~ sample,nucSizeLS))$sample %>% as.data.frame()
strsplit(rownames(tukLS),split="-") %>% unlist %>% matrix(.,byrow = T,ncol=2) %>% as.data.frame() -> x
tukLS <- cbind(x,tukLS)
...
TukeyNetwork(data.frame(sample=nucSizeLS$sample %>% gsub("Clone ","",.) %>% gsub("LS174T","CL",.),
                        value=nucSizeLS$area),pval = 0.01) -> clusters0


nucSizeLS$Group <- nucSizeLS$sample
levels(nucSizeLS$Group) <- clusters0$Groups$membership
aggregate(area ~ sample,nucSizeLS,m_cv)

TukeyHSD(aov(area ~ Group,nucSizeLS))
...
methylation <- read.delim("data/BisulfiteClonesData.tsv",sep="\t")
coors <- read.delim("data/coors.tsv",sep="\t")
melted0 <- melt(methylation[,c("Sample","Sequence",colnames(coors))]) 

melted0$Sample <- factor(melted0$Sample,c("LS174T",paste("Clone",1:14)))
...
size_meth <- data.frame(Area=mSize,Meth=bySample$value,Meth2=aggregate(value ~ Sample,melted0,mean)$value)

aov(Meth ~ I(Area>100),size_meth) %>% summary
t.test(Meth ~ I(Area>100),size_meth) 

aggregate(Meth ~ I(Area>100),size_meth,mean)
aggregate(Meth ~ I(Area>100),size_meth,sd)
...
summary(lm0 <- lm(Area ~ I(1/Meth),size_meth,subset=rownames(size_meth)!="LS174T"))
...
ggplot(size_meth,aes(x=Meth*100,y=Area)) + 
  ...
  scale_fill_meth +
  geom_label_repel(aes(label=c("LS174T",sprintf("C%02i",1:14))),nudge_x = .015,nudge_y = runif(14,-1,1)) +
  xlab("SST1 Average Methylation (%)") +
  ylab("Average nuclei area (μm2)") +
  annotate("text",42,145,label="P=0.003") +
  annotate("text",42,140,label="R^2==0.25",parse=T) +
  theme(text = element_text(size=12))


# Differences in cell cycle ----

cellCycle <- read.xls("data/CellCycleBEA.xls",sheet=1)
...
imppc <- merge(imppc,sst1.pairs,by.x="Patient",by.y="Case.number")


ggRegression(imppc$Normal,imppc$Tumor,group = imppc$KRAS,label = imppc$Patient) -> gkras
ggRegression(imppc$Normal,imppc$Tumor,group = imppc$BRAF,label = imppc$Patient) -> gbraf
ggRegression(imppc$Normal,imppc$Tumor,group = imppc$MSI,label = imppc$Patient)$plot +
...
gbraf$plot + 
  scale_color_manual(values = c("orange","green3","red3","red4")) + 
  xlab("SST1 RDL in Normal") +
  ylab("SST1 RDL in Tumor") +
  labs(colour="BRAF")
